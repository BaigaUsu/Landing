...
export async function DELETE(
  req: NextRequest,
  { params }: { params: { id: string; kind: string; stageId: string; subStageId: string; category: string; fileId: string } }
) {
  try {
    const { id, kind, stageId, subStageId, category, fileId } = params;
