...
export async function DELETE( req: NextRequest, { params }: { params: Promise<{ id: string; kind: string; stageId: string; subStageId: string }> }) {
    try {
      const { id, kind, stageId, subStageId } = await params;
